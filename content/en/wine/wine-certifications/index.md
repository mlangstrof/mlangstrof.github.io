...
We also need to consider that even if we found out that we prefer Primitivo from Puglia for example, it might be difficult to extrapolate from this to other wines. Imagine being in a restaurant that does not carry any Primitivos, what wine should we order? what wine should we order? Simply telling the waiter that we like Primitivos might help, but based solely on that, they cannot truly deduce what aspects of the wine appeal to us. For example, is it the residual sugar many Primitivos usually have? The low-medium acidity? The soft tannins? The (overripe/jammy) fruit aromas? There are many wines that feature some of these characteristics, but only you know what is most important to you.
